...
`codeeconomy.command.*`, сообщения под `codeeconomy.message.*`, подписи карточек и списков под
`codeeconomy.card.*`. Выдавать по маске `codeeconomy.admin.*` можно не опасаясь зацепить строки
перевода.
...
|---|---|---|
| `codeeconomy.balance` | `/balance`, `/bal`, `/money` | посмотреть свой баланс, в том числе показом на экране |
...
Нода `codeeconomy.balance` закрывает оба способа спросить свой баланс: и команду, и показ в углу
экрана у игрока с клиентской частью. Своей ноды у показа нет намеренно, иначе один и тот же вопрос
имел бы два разных ответа. Право спрашивается на каждой отправке, поэтому снятая нода гасит показ
без перезахода игрока.
