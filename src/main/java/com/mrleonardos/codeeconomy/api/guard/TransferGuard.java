package com.mrleonardos.codeeconomy.api.guard;

import com.mrleonardos.codeeconomy.api.model.AccountView;
import com.mrleonardos.codeeconomy.api.model.TransferRequest;

/**
 * Правило отказа до записи.
 *
 * <p>
 * Гвард смотрит на запрос и счета и решает, пускать ли операцию. У записи ещё нет: гвард отвечает до
 * обращения к хранилищу, поэтому его отказ не оставляет следов ни в журнале, ни в событиях. Отменить
 * операцию после записи гвард не может, для уведомлений служат события.
 *
 * <p>
 * Встроенный гвард один: кулдаун между переводами одного игрока. Личный потолок перевода из меты
 * {@code codeeconomy.paylimit} гвардом не проверяется, его считает сам конвейер до вызова цепочки и
 * отвечает кодом {@code ABOVE_CEILING}. Свой гвард чужой мод регистрирует в {@code EconomyApi} на
 * инициализации.
 */
public interface TransferGuard {

    /** Имя гварда, по нему он виден в логе, в том числе когда упал. */
    String id();

    /** Порядок в цепочке, меньшее число означает более ранний вызов. */
    int priority();

    /**
     * Проверить операцию.
     *
     * <p>
     * Метод обязан оставаться чистым: следов в гварде он не оставляет. Операция после него ещё может
     * не состояться, отказать способен и следующий гвард, и носитель, поэтому счётчики попыток,
     * кулдауны и квоты гвард ведёт в {@link #committed}, а не здесь.
     *
     * @param request что собираются сделать
     * @param from    счёт отправителя или null, когда у операции этой стороны нет
     * @param to      счёт получателя или null, когда у операции этой стороны нет
     */
    GuardResult check(TransferRequest request, AccountView from, AccountView to);

    /**
     * Операция записана на носителе.
     *
     * <p>
     * Здесь гвард отмечает то, что должно пережить операцию: время последнего перевода для кулдауна,
     * израсходованную квоту. Отказаться уже нельзя, деньги ушли. Исключение обрабатывается как
     * исключение из {@link #check}: имя гварда попадает в лог, операция не откатывается.
     */
    default void committed(TransferRequest request) {}
}
